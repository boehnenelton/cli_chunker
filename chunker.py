...
def run_chunk_txt(target_dir):
    ...
    out_dir = Path(config["output_base"]) / "chunked" / get_timestamp()
    out_dir.mkdir(parents=True, exist_ok=True)
    out_file = out_dir / f"{config['project_name']}.txt"
    ...
    try:
        ...
        print(f"\n[ERROR] Failed to save Text Chunk: {e}")

def run_unchunk_txt(txt_file):
    import re
    input_path = Path(txt_file).resolve()
    if not input_path.exists():
    ...
    try:
        ...
        if len(parts) < 2:
            print("Error: No file sections found in the text chunk.")
            return

        proj_name = input_path.stem
        out_dir = Path(DEFAULT_CONFIG["output_base"]) / "unchunked" / get_timestamp() / proj_name
        out_dir.mkdir(parents=True, exist_ok=True)
        
        file_count = 0
        # Parts will be [empty_or_preamble, filename1, content1, filename2, content2, ...]
...
def main():
    ...
    group.add_argument("--chunk-txt", metavar="DIR", help="Chunk a directory into a text file with separators")
    group.add_argument("--unchunk-txt", metavar="FILE", help="Unchunk a text file with separators into a directory")
